...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLSENS != cur->valueLSENS)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueLSENS Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagLSENS = 1;
            ret = 1;
        }
    }
    if(last->valueHOUR != cur->valueHOUR)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            ...
            gizwitsProtocol.waitReportDatapointFlag.flagHOUR = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagLSENS)
    {
        devStatusTmp.valueLSENS = gizY2X(LSENS_RATIO,  LSENS_ADDITION, dataPoints->valueLSENS); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueLSENS,LSENS_LEN);
        byteFieldOffset += LSENS_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagHOUR)
    {
        devStatusTmp.valueHOUR = gizY2X(HOUR_RATIO,  HOUR_ADDITION, dataPoints->valueHOUR); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueHOUR,HOUR_LEN);
        byteFieldOffset += HOUR_LEN;
    }
